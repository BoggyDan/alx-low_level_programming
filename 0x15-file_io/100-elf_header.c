 ...
 *
 * Description: if the file is not an ELF file, then exit with code 98
 */
void check_elf(unsigned char *e_ident)
{
	...
	for (idx = 0; idx < 4; idx++)
	{
		if (e_ident[idx] != 127 && e_ident[idx] != 'E' && e_ident[idx]
				!= 'L' && e_ident[idx] != 'F')
		{
 ...
 * @e_ident: pointer to array containing the ELF OS/ABI version
 */
...
/**
 * print_abi - prints the ABI version af an ELF header
 ...
 *
 * Description: if the file cannot be closed, then exit with code 98
 */
 ...
 * @ac: number of arcument supplied to the program
 * @av: array of pointers to the arguments
 *
 * Description: if the file is not an ELF file or on error, exit with code 98
 *
 * Return: always 0
 */
